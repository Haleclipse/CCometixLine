...
impl TrackedModel {
    ...
    fn default_color(&self) -> AnsiColor {
        match self {
...
/// Cache structure for CPA quota data
#[derive(Debug, Clone, Serialize, Deserialize)]
struct CpaQuotaCache {
...
#[derive(Default)]
pub struct CpaQuotaSegment;

impl CpaQuotaSegment {
    pub fn new() -> Self {
        Self
    }
    ...
    fn apply_foreground_color(&self, text: &str, color: &AnsiColor) -> String {
    ...
    fn get_cache_path() -> Option<std::path::PathBuf> {
        ...
        Some(
            home.join(".claude")
                .join("ccline")
                .join(".cpa_quota_cache.json"),
        )
    }

    fn load_cache(&self) -> Option<CpaQuotaCache> {
    ...
    fn save_cache(&self, cache: &CpaQuotaCache) {
    ...
    fn is_cache_valid(&self, cache: &CpaQuotaCache, cache_duration: u64) -> bool {
...
impl Segment for CpaQuotaSegment {
    fn collect(&self, _input: &InputData) -> Option<SegmentData> {
        // Load config from file to get segment options
        let config = crate::config::Config::load().ok()?;
        let segment_config = config.segments.iter().find(|s| s.id == SegmentId::CpaQuota);

        let empty_options = HashMap::new();
        let options = segment_config.map(|sc| &sc.options).unwrap_or(&empty_options);
